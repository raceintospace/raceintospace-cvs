...
void GetSoundDevice(char cfg_set)
{
  ...
  struct SndN {
   ...
   "Adlib                        \0",mADLIB,
   "Sound Blaster / Thunderboard \0",mADLIB,
...
getaddr:
       ...
       if (env==NULL) {// 0123456789012345678901
         ...
         gotoxy(2,9);cprintf("Configured...");clreol();

         //putenv(bString);
         fxin=fopen("BUZZ.BAT","w+t");

         fprintf(fxin,"@SET %s\n@BARIS.EXE %d %d\n",bString,Musics,Sounds);
         fclose(fxin);
         exit(0);
       }
     }
...
{
  FILE *fin;
  fin=fopen("test.tst","wb");
  if (fin) {
     fclose(fin);
     remove("test.tst");
  } else {
     printf("You cannot run this program from the BARIS CD-ROM.\n");
     printf("You must first install BARIS to your hard disk.");
     return;
  }
